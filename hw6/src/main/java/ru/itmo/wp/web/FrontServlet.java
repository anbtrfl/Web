...
//            throw new NotFoundException();
//        }
        Method beforeMethod = findMethod(pageClass, "before", HttpServletRequest.class, Map.class);
        Method method = findMethod(pageClass, route.getAction(), HttpServletRequest.class, Map.class);
        Method afterMethod = findMethod(pageClass, "after", HttpServletRequest.class, Map.class);
        ...
        try {
            invokeMethod(beforeMethod, pageClass, page, request, view);
            invokeMethod(method, pageClass, page, request, view);
            invokeMethod(afterMethod, pageClass, page, request, view);
...
//        }
    ...
    private Method findMethod(Class<?> pageClass, String methodName, Class<HttpServletRequest> httpServletRequestClass, Class<Map> mapClass) throws NotFoundException {
        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
    ...
    private void invokeMethod(Method method, Class<?> pageClass, Object page, HttpServletRequest request, Map<String, Object> view) throws ServletException, IOException {
        try {
            method.setAccessible(true);
            method.invoke(page, request, view);
        } catch (IllegalAccessException e) {
